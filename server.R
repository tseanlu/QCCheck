  ...
  observeEvent(input$QCsdtm,
               {
                 ...
                 Item2="General 2"
                 attach(data_sets())
                 if (exists("EPOCH") & !is.na(exists("EPOCH")))
                 {message2<-"EPOCH is included in the dataset." 
                 conclusion2<-"Passed"}
                 else {message2<-"EPOCH is not included in the dataset."
                 conclusion2<-"Failed"}
                 itemh2<-cbind(Item2,message2,conclusion2)
                 
                 Item3="General 3"
                 if (input$datasets=="pe.sas7bdat")
                 {
                   if (sum((data_sets()$PEDY==0))!=0
                       & !is.na(sum((data_sets()$PEDY==0))!=0)) #the variable maybe changed based on 
                     #different domain.
                   {message3<-"Error:the variable PEDY could not be zero."
                   conclusion3<-"Failed"}
                   else {message3<-"Correct. No value of PEDY is zero"
                   conclusion3<-"Passed"}
                 }
                 else {message3<-"This item is only applicable to PE domain."
                 conclusion3<-"Not applicable"}
                 ...
                 if (input$datasets=="dm.sas7bdat")
                 {
                   ...
                   if (df1$DTHFL=='Y') 
                   {message4<-"DTHFL has been mapped."
                   conclusion4<-"Passed"} 
                   else {message4<-"DTHFL has not been mapped." 
                   conclusion4<-"Failed"}
                 }
                 else {message4<-"This item is only applicable to DM domain."
                 conclusion4<-"Not applicable"}
                 itemh4<-cbind(Item4,message4,conclusion4) 
                 
                 Item5="General 4"
                 if (input$datasets=="pe.sas7bdat")
                 {
                   ep1<-data_sets()[which(data_sets()$PEDY<0),]
                   ep2<-data_sets()[which(data_sets()$PEDY>=0),]
                   ep3<-data_sets()[which(is.na(data_sets()$PEDY)),]
                   if ((sum(ep1$EPOCH!='SCREENING')==0) & 
                       (sum(!(ep2$EPOCH %in% c("FOLLOW-UP","TREATMENT")))==0)
                       & (sum(is.na(ep3$EPOCH))==0)) 
                   {message5<-"EPOCH's value is correct."
                   conclusion5<-"Passed"} 
                   else {message5<-"EPOCH's value is incorrect."
                   conclusion5<-"Failed"}
                 }
                 else {message5<-"This item is only applicable to PE domain."
                 conclusion5<-"Not applicable"}
                 itemh5<-cbind(Item5,message5,conclusion5)
                 
                 Item6="DM 2"
                 if (input$datasetr=="final.sas7bdat"
                     & input$datasets=="dm.sas7bdat")
                 {
                   fin1<-data_setr()[which(data_setr()$FINSTAT==11),]
                   # data_set is the raw dataset
                   b<-fin1[fin1$SUBJID %in% data_sets()$SUBJID,]
                   if ((sum(b$ACTARM!='Not Assigned')==0)
                       &(sum(b$ACTARMCD!='NOTASSGN')==0)) 
                   {message6<-"ACTARM and ACTARMCD are correct."
                   conclusion6<-"Passed"}
                   else {message6<-"ACTARM and ACTARMCD are incorrect."
                   conclusion6<-"Failed"}
                 }
                 else {message6<-"This item is only applicalbe to PDS raw and DM datasets."
                 ...
                 if (input$datasets=="dm.sas7bdat")
                 {
                   DMDTC<-data.frame(data_sets()$DMDTC)
                   ad<-format(DMDTC,format="%d-%b-%y") 
                   if (sum(data_sets()$DMDTC!=ad)==0) 
                   {message7<-"-DTC is in correct format." 
                   conclusion7<-"Passed"} 
                   else {message7<-"-DTC is not in correct format." 
                   conclusion7<-"Failed"}
                 }
                 ...
                 if (input$datasets=="dm.sas7bdat")
                 {
                   ...
                   if (res=="FALSE")
                   {message8<-"No other race in the dataset."
                   conclusion8<-"Passed"}
                   else {message8<-"Omg, there is other race in the dataset."
                   conclusion8<-"Failed"}
                 }
                 else {message8<-"This item is only applicable to DM domain."
                 ...
                 if (input$datasets=="suppdm.sas7bdat")
                 {
                   ...
                   if (mul=="FALSE")
                   {message9<-"No multiple race in the dataset."
                   conclusion9<-"Passed"}
                   else {message9<-"Omg, there is multiple race in the dataset."
                   conclusion9<-"Failed"}
                 }
                 else {message9<-"This item is only applicable to SUPPDM domain."
                 ...
                 if (input$datasets=="ex.sas7bdat")
                 {
                   ...
                   if (who=="FALSE")
                   {message10<-"It meets whodrug standard: PF-04383119."
                   conclusion10<-"Passed"}
                   else {message10<-"It didn't pass the QC check."
                   conclusion10<-"Failed"}
                 }
                 else {message10<-"This item is only applicable to EX domain."
                 ...
                 if (input$datasets=="lb.sas7bdat")
                 {
                   if (any(data_sets()$LBORRESU=="%"))
                   {
                     ...
                     {message11<-"It meets % rule: LBTESTCD comtain LE."
                     conclusion11<-"Passed"}
                     else {message11<-"Omg, check for LBTESTCD and LBTEST."
                     conclusion11<-"Failed"}
                   }
                 }
                 else {message11<-"This item in only applicable to LB domain."
                 conclusion11<-"Not applicable"}
                 itemh11<-cbind(Item11,message11,conclusion11)
                 
                 ## QC check for -HLT, -HLGT, -LLT legacy terms
                 Item12="General 6"
                     ...
                     input$datasets=="pr.sas7bdat")
                 {
                   if (input$datasets=="ae.sas7bdat")
                   {
                     ...
                     if (any((aevars$shaevars %in% sdtmvarnames$varsinsdtm)==FALSE))
                     {
                       ...
                       message12<-paste("The below required legacy terms are not in the AE dataset:",
                                        aenotsdtm2, ".")
                       conclusion12<-"Failed"
                     }
                     else {
                       message12<-"All hierachical legacy terms are included in the AE dataset."
                       conclusion12<-"Passed"
                     }
                   }
                   else if (input$datasets=="mh.sas7bdat")
                   {
                     ...
                     if (any((mhvars$shmhvars %in% sdtmvarnames$varsinsdtm)==FALSE))
                     {
                       ...
                       message12<-paste("The below required legacy terms are not in the MH dataset:",
                                        mhnotsdtm2, ".")
                       conclusion12<-"Failed"
                     }
                     else {
                       message12<-"All hierachical legacy terms are included in the MH dataset."
                       conclusion12<-"Passed"
                     }
                   }
                   else if (input$datasets=="pr.sas7bdat")
                   {
                     ...
                     if (any((prvars$shprvars %in% sdtmvarnames$varsinsdtm)==FALSE))
                     {
                       ...
                       message12<-paste("The below required legacy terms are not in the PR dataset:",
                                        prnotsdtm2, ".")
                       conclusion12<-"Failed"
                     }
                     else {
                       message12<-"All hierachical legacy terms are included in the PR dataset."
                       conclusion12<-"Passed"
                     }
                   }
                 }
                 else {message12<-"This item is only applicable to AE, MH or PR domain."
                 conclusion12<-"Not applicable"
                 }
                 message12<-sapply(lapply(message12, strwrap, width=50), paste, collapse="\n")
                     ...
                     input$datasets=="ae.sas7bdat")
                 {
                       ...
                       CRFspec1=="Collection date")
                   {
                     ...
                     if (any((prvars$shprvars %in% sdtmvarnames$varsinsdtm)==FALSE))
                     {
                       ...
                       prnotsdtm1<-as.data.frame(prnotsdtm)
                       prnotsdtm2<-apply(prnotsdtm1,2,paste,collapse=", ")
                       message13<-paste("The below variables are not mapped in the CM dataset:",
                                        prnotsdtm2, ".")
                       conclusion13<-"Failed"
                     }
                     else {
                       message13<-"The variables are mapped in the CM domain."
                       conclusion13<-"Passed"
                     }
                   }
                            ...
                            CRFspec1=="Collection date")
                   {
                     ...
                     if (any((prvars$shprvars %in% sdtmvarnames$varsinsdtm)==FALSE))
                     {
                       ...
                       prnotsdtm1<-as.data.frame(prnotsdtm)
                       prnotsdtm2<-apply(prnotsdtm1,2,paste,collapse=", ")
                       message13<-paste("The below variables are not mapped in the AE dataset:",
                                        prnotsdtm2, ".")
                       conclusion13<-"Failed"
                     }
                     else {
                       message13<-"The variables are mapped in the AE domain."
                       conclusion13<-"Passed"
                     }
                   }
                   ...
                   message13<-"This item is only applicable to CM or AE domain."
